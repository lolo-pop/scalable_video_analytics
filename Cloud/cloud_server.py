...
import logging
import cv2 as cv
from utils import (Results, Region, calc_iou, merge_images, extract_images_from_memory,
...
class CServer:
    ...
    def perform_single_image(self, file_data, point, binary_image):
        low_images_direc = "server_temp"
        cropped_images_direc = "server_temp-cropped"
        image = None
        import time
        start_time = time.time()
        if binary_image is True:
            image = skimage.io.imread(file_data)
        else:
