...
import logging
import cv2 as cv
from Edge.utils import (Results, Region, calc_iou, merge_images, extract_images_from_memory,
                       extract_images_from_video, merge_boxes_in_results,
                       compute_area_of_frame, calc_area, read_results_dict)
from Edge.object_detector import Detector
import skimage.io
import time
class Server:
    def __init__(self, gpu_id, model_path, nframes=None):
        # self.config = config
        ...
        self.detector = Detector(gpu_id, model_path)
    ...
    def perform_detection_np(self, video_np, resolution, fnums):
        final_results = Results()
        rpn_regions = Results()
        fid = self.curr_fid
        for i in range(fnums):
            detection_results, rpn_results = (
                self.detector.infer(video_np[i]))
            frame_with_no_results = True
            for label, conf, (x, y, w, h) in detection_results:
                # if (self.config.min_object_size and w * h < self.config.min_object_size) or w * h == 0.0:
                if (0 and w * h < 0) or w * h == 0.0:
                    continue
            ...
            self.logger.debug(
                ...
                f"and {len(rpn_regions)} for frame {fid}")
    ...
    def perform_detection(self, images_direc, resolution, fnames=None,
        ...
        return final_results, rpn_regions
        # return detection_results, rpn_regions
    ...
    '''
    def perform_video(self, vid_data):
        # Write video to file
        # with open(os.path.join("cserver_temp", "temp.mp4"), "wb") as f:
        ...
        end_fid = min(self.curr_fid + 15, 900)
        self.logger.info(f"Processing frames from {start_fid} to {end_fid}")
        req_regions = Results()
        for fid in range(start_fid, end_fid):
            req_regions.append(
                # Region(fid, 0, 0, 1, 1, 1.0, 2, self.config.low_resolution))
                Region(fid, 0, 0, 1, 1, 1.0, 2, 1))
        extract_images_from_video("cserver_temp", req_regions)

        fnames = [f for f in os.listdir("cserver_temp") if "png" in f]

        # results, rpn = self.perform_detection("cserver_temp", self.config.low_resolution, fnames)
        results, rpn = self.perform_detection("cserver_temp", 1, fnames)
        batch_results = Results()
        # batch_results.combine_results(results, self.config.intersection_threshold)
        batch_results.combine_results(results, 1)
        ...
        batch_results.combine_results(rpn, 1)
    ...
    def perform_video_np(self, vid_data):
        # Write video to file
        # with open(os.path.join("cserver_temp", "temp.mp4"), "wb") as f:
        #     f.write(vid_data.read())

        # Extract images
        video_np, fnums = extract_images_from_memory(vid_data)
        # Make req regions for extraction
        start_fid = self.curr_fid
        # end_fid = min(self.curr_fid + self.config.batch_size, self.nframes)
        end_fid = min(self.curr_fid + fnums, 900)
        self.logger.info(f"Processing frames from {start_fid} to {end_fid}")
        req_regions = Results()
        for fid in range(start_fid, end_fid):
            req_regions.append(
        ...
        # results, rpn = self.perform_detection("cserver_temp", self.config.low_resolution, fnames)
        results, rpn = self.perform_detection_np(video_np, fnums)
        batch_results = Results()
        # batch_results.combine_results(results, self.config.intersection_threshold)
        batch_results.combine_results(results, 1)
    ...
    def perform_single_image(self, image, point, binary_image):
        ...
        # start_time = time.time()
            ...
            # image = cv.imread(file_data)
            # image = tf.gfile.FastGFile(file_data, 'rb').read()
        # image = cv.cvtColor(image, cv.COLOR_BGR2RGB)
        ...
        # end_time = time.time()
        # start_time = time.time()
        # results = self.emulate_high_query(low_images_direc, low_images_direc, self.last_requested_regions)
        # detection_results, rpn_results = (self.detector.infer(image))
        output = (self.detector.infer(image, point))
        # end_time = time.time()
        # print("analyzing time:", end_time-start_time)
    ...
    def session_close(self):
        self.logger.info("session will be closed")
        self.detector.close()